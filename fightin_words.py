...
class FWExtractor(sk_base.BaseEstimator, sk_base.TransformerMixin):
    ...
    def transform(self, X, **params):
        ...
        #counts = self.cv.fit_transform([' '.join(X)]).toarray() --- WHY COMBINING?
        count_matrix = self.cv.fit_transform(X).toarray()
        ...
        if isinstance(self.prior, float):
            ...
            if (len(self.prior) == vocab_size):
                self.priors = np.array(self.prior)
            else:
        ...
        #count_matrix[0, :] = np.sum(counts[:len(X[0]), :], axis=0)  -- THIS IS A BUG!!!
